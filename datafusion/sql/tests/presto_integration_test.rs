...
mod tests {
    ...
    use datafusion_sql::presto::binding_context::{
        BindingContext, BindingContextStack, FloatAsDecimalContext,
        FunctionBindingContext,
    };
    use datafusion_sql::presto::function::{
        FunctionOverload, FunctionType, GenericDataType,
    };
    use datafusion_sql::presto::planner::{bind, BindResult};

    use datafusion_common::{DataFusionError, Result};
    use datafusion_common::{OwnedTableReference, TableReference};
    use datafusion_expr::logical_plan::LogicalPlan;
    use datafusion_expr::CreateExternalTable;
    use datafusion_expr::{LogicalPlanBuilder, TableSource};

    fn logical_plan(sql: &str, bc: Option<&BindingContextStack>) -> Result<LogicalPlan> {
        let table_binding_context = TableBindingContext::new();
        let function_binding_context = FunctionBindingContext::new(HashMap::from([
            (
                String::from("sum"),
                vec![Rc::new(FunctionOverload {
                    name: String::from("sum"),
                    type_: FunctionType::Aggregate,
                    return_type: GenericDataType::new_generic_primitive("T"),
                    argument_types: vec![GenericDataType::new_generic_primitive("T")],
                    variant_arguments: false,
                })],
            ),
            (
                String::from("min"),
            ...
            (
                ...
                vec![Rc::new(FunctionOverload {
                    ...
                    type_: FunctionType::Aggregate,
                    return_type: GenericDataType::DataType(DataType::Int64),
                    argument_types: vec![GenericDataType::new_generic_primitive("T")],
                    variant_arguments: false,
                })],
            ),
            (
                String::from("approx_median"),
                vec![Rc::new(FunctionOverload {
                    name: String::from("approx_median"),
                    type_: FunctionType::Aggregate,
                    return_type: GenericDataType::new_generic_primitive("T"),
                    argument_types: vec![GenericDataType::new_generic_primitive("T")],
                    variant_arguments: false,
            ...
            (
                ...
                vec![Rc::new(FunctionOverload {
                    ...
                    return_type: GenericDataType::DataType(DataType::Float64),
                    argument_types: vec![GenericDataType::DataType(DataType::Float64)],
                    variant_arguments: false,
                })],
            ),
            (
                ...
                vec![Rc::new(FunctionOverload {
                    ...
                    type_: FunctionType::Scalar,
                    return_type: GenericDataType::new_generic_array(Box::new(
                        GenericDataType::new_generic_primitive("T"),
                    )),
                    argument_types: vec![GenericDataType::new_generic_primitive("T")],
                    variant_arguments: true,
                })],
            ),
            (
                String::from("nullif"),
                vec![Rc::new(FunctionOverload {
                    name: String::from("nullif"),
                    type_: FunctionType::Scalar,
                    return_type: GenericDataType::new_generic_primitive("T"),
                    argument_types: vec![
                        GenericDataType::new_generic_primitive("T"),
            ...
            (
                ...
                vec![Rc::new(FunctionOverload {
                    ...
                    type_: FunctionType::Window,
                    return_type: GenericDataType::new(DataType::Int64),
                    argument_types: vec![],
                    variant_arguments: true,
                })],
            ),
            (
                ...
                vec![Rc::new(FunctionOverload {
                    ...
                    return_type: GenericDataType::new(DataType::Int64),
                    argument_types: vec![
                        GenericDataType::new(DataType::Float64),
                        GenericDataType::new(DataType::Int64),
                    ],
                    variant_arguments: false,
                })],
            ),
            (
                ...
                vec![Rc::new(FunctionOverload {
                    ...
                    type_: FunctionType::Scalar,
                    return_type: GenericDataType::new(DataType::Time64(
                        TimeUnit::Nanosecond,
                    )),
                    argument_types: vec![],
                    variant_arguments: false,
        ...
        let default = BindingContextStack::new(vec![
            &table_binding_context,
            &function_binding_context,
        ]);
        let bc = bc.unwrap_or(&default);
    ...
    impl TableBindingContext {
        ...
        fn get_schema(&self, schema: Cow<str>, table: Cow<str>) -> Result<Schema> {
            match (schema.as_ref(), table.as_ref()) {
                ("public", "person") => Ok(Schema::new(vec![
                ...
                ("public", "lineitem") => Ok(Schema::new(vec![
                    Field::new("l_item_id", DataType::UInt32, false),
                    Field::new("l_description", DataType::Utf8, false),
                    Field::new("price", DataType::Float64, false),
                ])),
                ("public", "aggregate_test_100") => Ok(Schema::new(vec![
                ...
                ("public", "orders") => Ok(Schema::new(vec![
        ...
        fn resolve_table(&self, table_ref: &TableReference) -> Result<LogicalPlan> {
            let schema = match &*table_ref {
                TableReference::Bare { table } => {
                    self.get_schema(Cow::from("public"), table.clone())
                }
                TableReference::Partial { schema, table } => {
                    self.get_schema(schema.clone(), table.clone())
                }
                TableReference::Full { .. } => todo!(),
            }?;
    ...
    fn test_with_binding_context(
        sql: &str,
        expected: &str,
        bc: Option<&BindingContextStack>,
    ) {
        match logical_plan(sql, bc) {
            Ok(plan) => assert_eq!(format!("{plan:?}"), expected),
            Err(e) => {
                println!("{}", e);
                panic!()
            }
    ...
    fn quick_test(sql: &str, expected: &str) {
        test_with_binding_context(sql, expected, None)
    }
    ...
    // ) -> LogicalPlan {
    //     let plan = logical_plan(sql, None).unwrap();
    ...
    fn parse_decimals() {
        ...
        for (a, b) in test_data {
            ...
            let float_as_decimal_context = FloatAsDecimalContext::new(true);
            let bc = BindingContextStack::new(vec![&float_as_decimal_context]);
            test_with_binding_context(&sql, &expected, Some(&bc));
        }
    }
    ...
    fn select_column_does_not_exist() {
        ...
        let err = logical_plan(sql, None).unwrap_err();
        let expected = "Bind(BinderError { row: 1, col: 7, message: \"No column doesnotexist found\" })";
    ...
    fn select_repeated_column() {
        ...
        let err = logical_plan(sql, None).unwrap_err();
        assert_eq!(
            r##"Bind(BinderError { row: 1, col: 12, message: "Projections require unique expression names but the expression (1, 7) person.age have the same name. Consider aliasing (\"AS\") one of them." })"##,
            format!("{err:?}")
        );
    }
    ...
    fn select_wildcard_with_repeated_column() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
            r##"Bind(BinderError { row: 1, col: 10, message: "Projections require unique expression names but the expression (1, 7) person.age have the same name. Consider aliasing (\"AS\") one of them." })"##,
            format!("{err:?}")
        );
    }
    ...
    fn select_filter_column_does_not_exist() {
        ...
        let err = logical_plan(sql, None).unwrap_err();
        let expected = "Bind(BinderError { row: 1, col: 36, message: \"No column doesnotexist found\" })";
        assert_eq!(expected, format!("{err:?}"));
    }
    ...
    fn select_filter_cannot_use_alias() {
        ...
        let err = logical_plan(sql, None).unwrap_err();
        let expected =
            "Bind(BinderError { row: 1, col: 41, message: \"No column x found\" })";
        assert_eq!(expected, format!("{err:?}"));
    }
    ...
    fn table_with_column_alias_number_cols() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
            r##"Bind(BinderError { row: 2, col: 39, message: "Source table contains 3 columns but only 2 names given as column alias" })"##,
            format!("{err:?}")
        );
    }
    ...
    fn select_with_ambiguous_column() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
            "Bind(BinderError { row: 1, col: 7, message: \"Ambiguous column id found\" })",
            format!("{err:?}")
        );
    }
    ...
    fn select_with_having() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
                "Bind(BinderError { row: 3, col: 30, message: \"HAVING clause references: person.age > Int64(100) AND person.age < Int64(200) must appear in the GROUP BY clause or be used in an aggregate function\" })",
                format!("{err:?}")
            );
    }
    ...
    fn select_with_having_referencing_column_not_in_select() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
                "Bind(BinderError { row: 3, col: 30, message: \"HAVING clause references: person.first_name = Utf8(\\\"M\\\") must appear in the GROUP BY clause or be used in an aggregate function\" })",
                format!("{err:?}")
            );
    }
    ...
    fn select_with_having_refers_to_invalid_column() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
                "Bind(BinderError { row: 4, col: 30, message: \"Non-aggregate values are referenced: Expression person.first_name could not be resolved from available columns: person.id, MAX(person.age)\" })",
                format!("{err:?}")
            );
    }
    ...
    fn select_with_having_referencing_column_nested_in_select_expression() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
                "Bind(BinderError { row: 3, col: 30, message: \"HAVING clause references: person.age > Int64(100) must appear in the GROUP BY clause or be used in an aggregate function\" })",
                format!("{err:?}")
            );
    }
    ...
    fn select_with_having_with_aggregate_not_in_select() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
                "Bind(BinderError { row: 1, col: 7, message: \"Non-aggregate values are referenced: Expression person.first_name could not be resolved from available columns: MAX(person.age)\" })",
                format!("{err:?}")
            );
    }
    ...
    fn select_aggregate_with_having_referencing_column_not_in_select() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
            "Bind(BinderError { row: 3, col: 30, message: \
                \"Non-aggregate values are referenced: Expression person.first_name \
                could not be resolved from available columns: COUNT(UInt8(1))\" })",
            format!("{err:?}")
        );
    }
    ...
    fn select_aggregate_with_group_by_with_having_referencing_column_not_in_group_by() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
            "Bind(BinderError { row: 4, col: 30, message: \"\
            Non-aggregate values are referenced: Expression person.last_name could not be \
            resolved from available columns: person.first_name, MAX(person.age)\" })",
            format!("{err:?}")
        );
    }
    ...
    fn select_simple_aggregate_column_does_not_exist() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
            "Bind(BinderError { row: 1, col: 11, message: \"No column doesnotexist found\" })",
            format!("{err:?}")
        );
    }
    ...
    fn select_simple_aggregate_repeated_aggregate() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
            "Bind(BinderError { row: 1, col: 17, message: \"Projections require unique \
            expression names but the expression (1, 7) MIN(person.age) have the same name. \
    ...
    fn select_simple_aggregate_repeated_aggregate_with_repeated_aliases() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
            "Bind(BinderError { row: 1, col: 22, message: \"Projections require unique expression names but the expression \
            (1, 7) a have the same name. Consider aliasing (\\\"AS\\\") one of them.\" })",
            format!("{err:?}")
        );
    }
    ...
    fn select_simple_aggregate_with_groupby_with_aliases_repeated() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
            r##"Bind(BinderError { row: 1, col: 19, message: "Projections require unique expression names but the expression (1, 7) a have the same name. Consider aliasing (\"AS\") one of them." })"##,
            format!("{err:?}")
        );
    }
    ...
    fn select_simple_aggregate_with_groupby_and_column_in_group_by_does_not_exist() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
            "Bind error: line 1:37 No column doesnotexist found",
            format!("{err}")
        );
    }
    ...
    fn select_simple_aggregate_with_groupby_and_column_in_aggregate_does_not_exist() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
            "Bind error: line 1:11 No column doesnotexist found",
            format!("{err}")
        );
    }
    ...
    fn select_array_no_common_type() {
        let sql = "SELECT array[1, true, null]"; // syntax change
        let err = logical_plan(sql, None).expect_err("query should have failed");

        assert_eq!(
            "Bind(BinderError { row: 1, col: 16, message: \"\
            Arrays with different types are not supported: Boolean != Some(Int64)\" })",
            format!("{err:?}")
        );
    }
    ...
    #[test]
    fn recursive_ctes() {
        let sql = "WITH RECURSIVE numbers AS (
                  select 1 as n
                UNION ALL
    ...
    fn select_array_non_literal_type() {
        let sql = "SELECT array[now()]"; // syntax change
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
            "Bind(BinderError { row: 1, col: 13, message: \"Arrays with elements other than literal are not supported\" })",
            format!("{err:?}")
        );
    }
    ...
    fn select_simple_aggregate_with_groupby_position_out_of_range() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
                "Bind(BinderError { row: 1, col: 44, message: \"Projection references non-aggregate values: ordinal 0 could not be resolved\" })",
                format!("{err:?}")
            );

        let sql2 = "SELECT state, MIN(age) FROM person GROUP BY 5";
        let err2 = logical_plan(sql2, None).expect_err("query should have failed");
        assert_eq!(
                "Bind(BinderError { row: 1, col: 44, message: \"Projection references non-aggregate values: ordinal 5 could not be resolved\" })",
                format!("{err2:?}")
            );
    }
    ...
    fn select_simple_aggregate_with_groupby_aggregate_repeated() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
            r##"Bind(BinderError { row: 1, col: 24, message: "Projections require unique expression names but the expression (1, 14) MIN(person.age) have the same name. Consider aliasing (\"AS\") one of them." })"##,
            format!("{err:?}")
        );
    }
    ...
    fn select_simple_aggregate_with_groupby_non_column_expression_nested_and_not_resolvable(
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
                "Bind(BinderError { row: 1, col: 7, message: \"Non-aggregate values are referenced: Expression person.age could not be resolved from available columns: person.age + Int64(1), MIN(person.first_name)\" })",
                format!("{err:?}")
            );
    }
    ...
    fn select_simple_aggregate_with_groupby_non_column_expression_and_its_column_selected(
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!("Bind(BinderError { row: 1, col: 7, message: \"Non-aggregate values are referenced: Expression person.age could not be resolved from available columns: person.age + Int64(1), MIN(person.first_name)\" })",
                format!("{err:?}")
            );
    }
    ...
    fn select_order_by_index_of_0() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
            "Bind(BinderError { row: 1, col: 31, message: \
                \"Projection references non-aggregate values: \
                ordinal 0 could not be resolved\" })",
            format!("{err:?}")
        );
    }
    ...
    fn select_order_by_index_oob() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
            "Bind(BinderError { row: 1, col: 31, message: \"Projection references non-aggregate values: ordinal 2 could not be resolved\" })",
            format!("{err:?}")
        );
    }
    ...
    fn select_7480_2() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
            "Bind(BinderError { row: 1, col: 11, message: \"Non-aggregate \
                values are referenced: Expression aggregate_test_100.c13 could \
                not be resolved from available columns: aggregate_test_100.c1, \
    ...
    fn create_external_table_csv() {
        // let sql = "CREATE EXTERNAL TABLE t(c1 int) STORED AS CSV LOCATION 'foo.csv'"; // syntax change
        let sql =
            "CREATE TABLE t(c1 int) WITH (FORMAT='CSV', EXTERNAL_LOCATION='foo.csv') ";
    ...
    fn create_external_table_custom() {
        // let sql = "CREATE EXTERNAL TABLE dt STORED AS DELTATABLE LOCATION 's3://bucket/schema/table';"; // Syntax change
        let sql =
            "CREATE TABLE dt WITH (FORMAT='DELTATABLE', EXTERNAL_LOCATION='s3://bucket/schema/table')";
        if let LogicalPlan::CreateExternalTable(CreateExternalTable {
            schema,
            name: OwnedTableReference::Bare { table },
            location,
    ...
    fn create_external_table_csv_no_schema() {
        // let sql = "CREATE EXTERNAL TABLE t STORED AS CSV LOCATION 'foo.csv'"; // Syntax change
        let sql = "CREATE TABLE t WITH (FORMAT='CSV', EXTERNAL_LOCATION='foo.csv')";
        if let LogicalPlan::CreateExternalTable(CreateExternalTable {
            schema,
            name: OwnedTableReference::Bare { table },
            location,
    ...
    fn create_external_table_with_compression_type() {
        let sqls = vec![ // Syntax change
            ("CREATE TABLE t(c1 int) WITH(FORMAT='CSV', COMPRESSION='GZIP', EXTERNAL_LOCATION='foo.csv.gz')", "CSV", CompressionTypeVariant::GZIP, "foo.csv.gz"),
            ("CREATE TABLE t(c1 int) WITH(FORMAT='CSV', COMPRESSION='BZIP2', EXTERNAL_LOCATION='foo.csv.bz2')", "CSV", CompressionTypeVariant::BZIP2, "foo.csv.bz2"),
            ("CREATE TABLE t(c1 int) WITH(FORMAT='JSON', COMPRESSION='GZIP', EXTERNAL_LOCATION='foo.json.gz')", "JSON", CompressionTypeVariant::GZIP, "foo.json.gz"),
            ("CREATE TABLE t(c1 int) WITH(FORMAT='JSON', COMPRESSION='BZIP2', EXTERNAL_LOCATION='foo.json.bz2')", "JSON", CompressionTypeVariant::BZIP2, "foo.json.bz2"),
        ];
        ...
        // negative case
        let sqls = vec![ // Syntax change
            "CREATE TABLE t WITH(FORMAT='AVRO', COMPRESSION='GZIP', EXTERNAL_LOCATION='foo.avro')",
            "CREATE TABLE t WITH(FORMAT='AVRO', COMPRESSION='BZIP2', EXTERNAL_LOCATION='foo.avro')",
            "CREATE TABLE t WITH(FORMAT='PARQUET', COMPRESSION='GZIP', EXTERNAL_LOCATION='foo.parquet')",
            "CREATE TABLE t WITH(FORMAT='PARQUET', COMPRESSION='BZIP2', EXTERNAL_LOCATION='foo.parquet')",
        ];
        for sql in sqls {
            let err = logical_plan(sql, None).expect_err("query should have failed");
            assert_eq!(
                "Bind(BinderError { row: 1, col: 19, message: \"File compression type can be specified for CSV/JSON files.\" })",
                format!("{err:?}")
            );
        }
    }
    ...
    fn create_external_table_parquet() {
        // let sql = "CREATE EXTERNAL TABLE t(c1 int) STORED AS PARQUET LOCATION 'foo.parquet'"; // Syntax change
        let sql = "CREATE TABLE t(c1 int) WITH (FORMAT='PARQUET', EXTERNAL_LOCATION='foo.parquet')";
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
            "Bind(BinderError { row: 1, col: 15, message: \"File compression type can be specified for CSV/JSON files.\" })",
            format!("{err:?}")
        );
    }
    ...
    fn create_external_table_parquet_no_schema() {
        // let sql = "CREATE EXTERNAL TABLE t STORED AS PARQUET LOCATION 'foo.parquet'"; // Syntax change
        let sql =
            "CREATE TABLE t WITH (FORMAT='PARQUET', EXTERNAL_LOCATION='foo.parquet')";
        if let LogicalPlan::CreateExternalTable(CreateExternalTable {
            schema,
            name: OwnedTableReference::Bare { table },
            location,
    ...
    fn union_with_incompatible_data_type() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
            "Bind(BinderError { row: 1, col: 31, message: \"UNION Column Int64(1) \
            (type: Int64) is not compatible with column IntervalMonthDayNano\
    ...
    fn union_with_incompatible_data_types() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
                "Bind(BinderError { row: 1, col: 13, message: \"UNION \
                Column a (type: Boolean) is not compatible with column a (type: Utf8)\" })",
                format!("{err:?}")
            );
    }
    ...
    fn over_order_by_with_window_frame_range_order_by_check() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
                "Bind(BinderError { row: 1, col: 32, message: \"With window frame of type RANGE, the order by expression must be of length 1, got 0\" })",
                format!("{err:?}")
            );
    }
    ...
    fn over_order_by_with_window_frame_range_order_by_check_2() {
        ...
        let err = logical_plan(sql, None).expect_err("query should have failed");
        assert_eq!(
                "Bind(BinderError { row: 1, col: 32, message: \"With window frame of type RANGE, the order by expression must be of length 1, got 2\" })",
                format!("{err:?}")
            );
    }
    ...
    fn cte_use_same_name_multiple_times() {
        ...
        let expected =
            "Bind error: line 1:34 WITH query name \"a\" specified more than once";
        let result = logical_plan(sql, None).err().unwrap();
    ...
    fn cte_unbalanced_number_of_columns() {
        ...
        let expected = "Bind error: line 1:12 Source table contains 3 columns but only 1 names given as column alias";
        let result = logical_plan(sql, None).err().unwrap();
        assert_eq!(result.to_string(), expected);
    }
    ...
    fn hive_aggregate_with_filter() -> Result<()> {
        ...
        let plan = logical_plan(sql, None)?;
    ...
    //     let single_quoted_res = format!("{:?}", logical_plan("SELECT '1'", None).unwrap());
    //     let double_quoted_res = format!("{:?}", logical_plan("SELECT \"1\"", None).unwrap());
    //     assert_eq!(single_quoted_res, double_quoted_res);

    //     // It should return error in other dialect.
    //     assert!(logical_plan("SELECT \"1\"", None).is_err());
    // }
    ...
    fn test_ambiguous_column_references_in_on_join() {
        ...
        let expected = "Bind error: line 4:19 Ambiguous column id found";

        // It should return error.
        let result = logical_plan(sql, None);
