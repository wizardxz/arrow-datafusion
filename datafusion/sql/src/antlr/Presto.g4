...
singleStatement options {logical='LogicalPlan';}
    : statement EOF
    ;
...
statement options {logical='LogicalPlan';}
    ...
    | ALTER TABLE (IF EXISTS)? tableName=qualifiedName
        ALTER COLUMN columnName=identifier SET DATA TYPE type_          #setColumnType
    ...
    | UPDATE qualifiedName
        ...
        (WHERE where_=booleanExpression)?                               #update
...
tableElement options {logical='Field';}
    : columnDefinition
    | likeClause
    ;
...
propertyValue options {logical='Expr';}
    : DEFAULT       #defaultPropertyValue
    | expression    #nonDefaultPropertyValue
    ;

queryNoWith options {logical='LogicalPlan';}
    : queryTerm
...
queryTerm options {logical='LogicalPlan';}
    : queryPrimary                                                             #queryTermDefault
    | left=queryTerm operator=INTERSECT setQuantifier? right=queryTerm         #setOperation
    | left=queryTerm operator=(UNION | EXCEPT) setQuantifier? right=queryTerm  #setOperation
    ;

queryPrimary options {logical='LogicalPlan';}
...
sortItem options {logical='Expr';}
    : expression ordering=(ASC | DESC)? (NULLS nullOrdering=(FIRST | LAST))?
    ;

querySpecification options {logical='LogicalPlan';}
    : SELECT setQuantifier? querySelectItems
...
groupingElement options {logical='GroupingElement';}
...
groupingSet options {logical='GroupingSet';}
...
namedQuery options {logical='LogicalPlan';}
...
selectItem options {logical='Expr';}
    : expression (AS? identifier)?                          #selectSingle
    | primaryExpression '.' ASTERISK (AS columnAliases)?    #selectAll
    | ASTERISK                                              #selectAll
    ;

relation options {logical='LogicalPlan';}
    : left=relation
...
sampledRelation options {logical='LogicalPlan';}
    : patternRecognition (
...
patternRecognition options {logical='LogicalPlan';}
    : aliasedRelation (
        MATCH_RECOGNIZE '('
          (PARTITION BY partition+=expression (',' partition+=expression)*)?
          (ORDER BY sortItem (',' sortItem)*)?
...
aliasedRelation options {logical='LogicalPlan';}
    : relationPrimary (AS? identifier columnAliases?)?
    ;

columnAliases options {logical='Vec<String>';}
    : '(' identifier (',' identifier)* ')'
    ;

relationPrimary options {logical='LogicalPlan';}
...
expression options {logical='Expr';}
    : booleanExpression
    ;

booleanExpression options {logical='Expr';}
...
predicate options {logical='Expr';}
...
valueExpression options {logical='Expr';}
...
primaryExpression options {logical='Expr';}
    : NULL                                                                                #nullLiteral
...
type_ options {logical='DataType';}
    : ROW '(' rowField (',' rowField)* ')'                                         #rowType
    | INTERVAL from=intervalField (TO to=intervalField)?                           #intervalType
    | base_=TIMESTAMP ('(' precision = typeParameter ')')? (WITHOUT TIME ZONE)?     #dateTimeType
    | base_=TIMESTAMP ('(' precision = typeParameter ')')? WITH TIME ZONE           #dateTimeType
    | base_=TIME ('(' precision = typeParameter ')')? (WITHOUT TIME ZONE)?          #dateTimeType
    | base_=TIME ('(' precision = typeParameter ')')? WITH TIME ZONE                #dateTimeType
...
identifier options {logical='String';}
